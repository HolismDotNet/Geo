using System;
using Microsoft.EntityFrameworkCore.Migrations;

namespace Holism.Geo.DataAccess.Migrations
{
    public partial class Init : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.CreateTable(
                name: "Countries",
                columns: table => new
                {
                    Id = table.Column<long>(nullable: false)
                        .Annotation("SqlServer:Identity", "1, 1"),
                    Guid = table.Column<Guid>(nullable: false),
                    Name = table.Column<string>(nullable: true),
                    Capital = table.Column<string>(nullable: true),
                    OfficialName = table.Column<string>(nullable: true),
                    AlternativeName = table.Column<string>(nullable: true),
                    IsoTwoLetterCode = table.Column<string>(nullable: true),
                    IsoThreeLetterCode = table.Column<string>(nullable: true),
                    IsoNumericCode = table.Column<string>(nullable: true),
                    Cctld = table.Column<string>(nullable: true)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_Countries", x => x.Id);
                });
        }

        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DropTable(
                name: "Countries");
        }
    }
}
